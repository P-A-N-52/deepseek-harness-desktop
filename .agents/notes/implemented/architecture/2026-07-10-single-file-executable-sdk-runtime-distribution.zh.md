...
> 实测（macos-arm64、Node 24.19.0 构建目标、pkg 6.21.0）：VFS 内裸包名 ESM 动态 `import()`（含顶层 `await`）、CJS 互操作、`node:sqlite`、集合外包名明确报错、VFS 外磁盘 ESM `import()` 全部通过，`import.meta.url` 原样为 `file:///snapshot/...`。

`--sea` 要求构建目标 ≥ Node 22，exe 统一以精确的 Node 24.19.0 版本为构建目标；每次 pkg 调用只打包一个构建目标，多平台各调用一次。
...
[`scripts/single-exe-build.ts`](../../../../scripts/single-exe-build.ts) 统一负责 [`scripts/build-exe-for-python-sdk.ts`](../../../../scripts/build-exe-for-python-sdk.ts) 与 Desktop 运行时构建器使用的闭包和 SEA 流水线。每个纯依赖部署 manifest 都声明 `files: []`，因此部署时不会把仓库内的构建缓存复制进运行时。流水线先校验运行时 peer 闭包并构建工作区，要求根目录 `packageManager` 声明与实际解析到的 `pnpm` 可执行文件都精确为 11.7.0，再清空产品暂存目录，通过仅对该命令启用的工作区注入执行冻结、离线且非 legacy 的部署。该部署从根 lockfile 导出产品闭包；`node-linker=hoisted` 只为 pkg 提供稳定的单实例文件布局，不允许绕过 lock 或回退到注册表。流水线保留每个嵌套符号链接，直到该链接自己的校验轮次；每个解析目标都必须位于部署根目录下，包管理器的 `.bin` 目录会被删除，任何无法安全物化的链接都会使构建失败。依赖脚本继续受根目录 `allowBuilds` 策略约束；产出的 `.modules.yaml` 必须只报告一个被跳过的脚本，即本地 `@deepseek-ai/dsh-subprocess-local` 的 postinstall。流水线校验包名、脚本文本、普通文件类型与暂存目录包含关系后，通过当前 Node 可执行文件直接运行该暂存脚本，再删除包管理器元数据。随后，流水线注入产品的 pkg 入口与完整动态资产清单，暂存目标平台的 `node-pty` addon，校验已安装且带专用补丁的 `@yao-pkg/pkg@6.21.0` 包，通过 Node 直接调用它唯一解析到的 bin，并只把经过 SHA-256 校验的官方 Node 24.19.0 固定归档放入供 pkg 使用的全新私有缓存。Linux CI 会在匹配的 manylinux 2.28 环境中重新构建 `pty.node`，再把该构建复制进可移植闭包；macOS 使用目标平台预构建产物，并生成所需的 `-spawn-helper`。Python 产品 `dsh-jsonrpc-agent-pkg-<platform>-<arch>` 写入 `dist-exe/` 后复制进运行时载体；CI 保留平台 wheel，而不保留中间裸可执行文件。
