...
`linux` 泳道复用一次不可变安装与构建，但会按所有权顺序执行两个阶段：通用仓库聚合必须成功，之后才提供 Chromium 并开始专用 Web 浏览器回放。通用阶段和浏览器阶段分别设置 50 分钟与 30 分钟时限，既能明确归因停滞阶段，又不重复 checkout 和依赖设置；110 分钟的作业总时限把余下 30 分钟留给安装与环境准备。

通用门禁调度器会在必需门禁失败后停止启动新工作，并采用[并行门禁决策](2026-07-06-parallel-pre-push-gates.md)所规定的、必须等待完成的强制取消生命周期。主 CI 有意只把该生命周期用于 CI 已经出现阻断失败后被丢弃的兄弟门禁；正常命令完成和应用退出仍保留各自的优雅生命周期。
...
- **先优雅终止失败运行的兄弟门禁，再进行强制回收** — 否决：终止信号处理器可以在调度器捕获进程树后创建脱离的后代，使已经无法影响 CI 结论的工作不能在有界时间内完成回收。

## 后果

贡献者可以在普通 GitHub 托管镜像和不可变安装上复现必需作业清单。排队时间和性能变化通过普通运行观察，而非通过保留的基准测试机群。

必需门禁失败后会保留其首个结果，同时等待兄弟门禁完成取消。回收失败会与该结果一同报告，不会被误判为成功取消。
