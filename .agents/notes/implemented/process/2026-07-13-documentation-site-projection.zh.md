...
网站构建与外部发布保持分离。文档工作流会使用既有的冻结安装和 `doc-sync` 构建，以 VitePress 的根 base path 验证每项受选中的变更。它不会配置 Pages、上传产物、部署站点，也不会获得 Pages 或 OIDC 权限。[不使用 Pages 的私有文档验证](2026-08-14-private-documentation-verification-without-pages.md)负责发布边界。网站投影仍与目标位置无关，但发布需要另一项明确授权的决定和实现。
...
文档事实只有一个可编辑归属，公开路由在源文件移动后仍保持稳定，网站也能纳入生成的参考资料而无需提交另一份生成副本。本地开发会监视权威输入并重新生成一次性投影。布局门禁会把陈旧的网站专用 Markdown 目录树变成合并失败，而不是被忽略的构建输入。影响文档网站的合并会验证检查过的结果，手动触发则提供不发布内容的恢复和验证入口。
