...
根据[浏览器快照 CI 决策](2026-07-30-web-browser-snapshot-ci-gate.md)，该车道是主 `linux` CI 泳道中的必需只比较门禁。`ci-linux-primary` 会构建当前仓库，安装锁文件选定的 Chromium 及其托管系统依赖，并用 `DSH_SNAPSHOT=replay` 运行该车道。这是有意的平面切分：host 与 spec 使用 [tsx 源码启动约定](../architecture/2026-07-29-dsh-source-launch-tsx-esm.md)，浏览器则在 `built-package-invariants` 验证后消费 `apps/web/dist` 和包的 `lib/client.js` 产物。CI 从不录制或刷新预期输出。场景仍面向 POSIX，并置于原生 Windows 泳道之外。
