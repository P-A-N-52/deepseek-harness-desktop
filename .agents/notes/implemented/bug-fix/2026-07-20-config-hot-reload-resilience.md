...
HMR contains live refresh rejection. Its `registerConfig(filename, refresh)` method canonicalizes one exact filename through the deepest existing ancestor, stat-polls only that file at a validated interval, and explicitly samples the initial state. A missing path is idle, while an existing file is scheduled once; later creation, change, removal, and recreation are observed without relying on recursive directory discovery. Exact-path and ordinary config-file refreshes use the same serialized, coalescing queue. The async disposer stops polling before it drains initial reconciliation and active work. A failure is normalized to `Error`, logged, and broadcast through the parallel `hmr/config-update-failed(filename, error)` event; rejecting observers are logged without stopping later refreshes.
...
- Each registered exact config path owns one bounded stat poller, controlled by the validated HMR interval, and releases it with the owning HMR fiber.
