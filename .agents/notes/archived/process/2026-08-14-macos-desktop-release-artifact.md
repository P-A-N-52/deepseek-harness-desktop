...
Credential-free CI starts without generated legal resources, builds the application on an Apple Silicon runner, checks the generated evidence, starts the final SEA in Chromium, and verifies native host ownership and restart. It neither reads Apple signing credentials nor publishes assets.

Public release is a manual-only GitHub Actions workflow on the exact annotated Desktop tag. The `desktop-release` environment owns the publisher-repository switch, required reviewer and tag policy, Developer ID certificate, App Store Connect notarization key, signing identity, and Team ID. The workflow requires the tag commit to be reachable from the protected default branch and rechecks that the remote tag object is unchanged before publication. It repeats the release-shaped build and acceptance checks before importing credentials into an ephemeral keychain, then invokes the same packager and verifies the three output assets. GitHub receives a draft containing only the DMG, `SHA256SUMS`, and `release-manifest.json`; the workflow makes it public only after all uploads complete. A retry reuses only a draft whose three downloaded assets are byte-identical to the new outputs. A push or tag creation alone cannot read credentials or publish an artifact.
