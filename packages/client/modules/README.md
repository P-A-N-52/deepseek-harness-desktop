...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves their metadata through the application deployment's `ctx.appModuleResolver`, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and serves it with its source map under `/plugins`. The resolver is the same package tree app boot selected for bare plugin imports: the profile installation for an extensible source launch or the executable closure for a sealed runtime. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
