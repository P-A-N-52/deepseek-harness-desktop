...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，通过应用部署的 `ctx.appModuleResolver` 解析其元数据，再解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。该解析器与 app boot 为裸插件导入选定的包树相同：可扩展的源码启动使用 profile 安装，封闭运行时使用可执行文件闭包。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
