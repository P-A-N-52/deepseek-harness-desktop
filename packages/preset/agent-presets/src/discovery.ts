...
/**
 * Whether `path` names an existing directory rather than a symlink.
 * @param path - Absolute path to test.
 * @returns true when the path itself is a directory.
 */
async function isDirectory(path: string): Promise<boolean> {
  try {
    return (await lstat(path)).isDirectory()
  } catch {
    // A root can change between enumeration and inspection. The vanished or
    // unreadable child no longer occupies a discoverable preset slot.
    return false
  }
}
