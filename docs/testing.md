...
- **Real-API e2e** (`pnpm run test:e2e`): with-key tests against live provider APIs — the DeepSeek model plus provider-specific smokes that gate on their own keys (`EXA_API_KEY`, `PERPLEXITY_API_KEY`, …). Suites self-skip locally without their key; CI does not run them automatically. A maintainer may dispatch the separate workflow, whose missing-DeepSeek-key preflight fails loudly rather than reporting a false green ([manual real-API e2e Agent Note](../.agents/notes/implemented/testing/2026-08-14-manual-real-api-e2e-for-independent-desktop.md)).
- **Snapshot** (`pnpm run test:snapshot`): keyless expected outputs cover external behavior — transport contracts and presentation, while persisted logs pin assembled backend behavior. ACP boots the real automation-server example, replays a recorded session, and diffs normalized JSON-RPC plus the re-persisted log ([ACP snapshot Agent Note](../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.md)); headless backend scenarios boot their explicit example composition through an unexported JSONL test driver, while `apps/cli` separately owns product `dsh --profile headless` acceptance. Use `pnpm run test:snapshot:record` when a model transcript changes and `pnpm run test:snapshot:refresh` when replay input remains valid; review every JSONL and expected-output diff. The `text-turn` ACP scenario pins complete prompt/schema content; tokenized peers localize churn ([pinned-header Agent Note](../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)). This config excludes `apps/web`.
- **Web browser snapshot** (`pnpm run test:web`; required Linux PR gate): the dedicated Vitest config solely owns replay under `apps/web/tests/snapshots/`. After the general Linux gates pass, CI runs it with read-only `DSH_SNAPSHOT=replay`; recording remains local and reviewed ([web e2e lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [CI gate decision](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)). `test:web` [builds first](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md) for plugin CSS.
