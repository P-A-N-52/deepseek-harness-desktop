...
包加入这张表的方式，是在自己的 package.json 中声明 `dsh.client`（`platform: 'web'`、可选的 `inject` 边、可选的 `immediately`），并在 `exports["./client"]` 导出构建好的 bundle。包元数据通过 `ctx.appModuleResolver` 解析，即 app boot 为裸插件导入选定的包树。可扩展启动使用 profile 安装，封闭运行时使用可执行文件闭包；被包含树的 `ctx.baseUrl` 则继续指向物理 profile 目录，用于解析相对配置行。
