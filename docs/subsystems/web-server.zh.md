...
[dsh-host-webserver](../../packages/host/webserver) 是 GUI 宿主的浏览器 HTTP 载体：它是一个提供 `ctx.webServer` 的 `node:http` 插件，包含具名路由注册表、index.html 转换回调，以及一个可由插件认领的回退处理器。它不属于 agent loop（智能体循环），也不是能力 seam；它不了解任何 harness 概念。其他插件负责注册所有功能路由，包括 `/api` 桥接、插件 bundle 和 HMR（热模块替换）事件流（[分层说明](../../.agents/notes/implemented/architecture/2026-07-19-gui-layering-and-rpc-protocol.md)）。发布的 Web profile 和 [Desktop 应用](../../apps/desktop/README.md)都经回环 HTTP URL 加载它。Desktop shell 只打开 sidecar 报告的 `127.0.0.1` origin，因此页面 fetch 和 WebSocket 流继续使用浏览器同源载体及既有的 [API 信任栅栏](../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md)；它不提供 Tauri IPC 载体或 remote capability（见 [Desktop 回环说明](../../.agents/notes/implemented/architecture/2026-08-13-tauri-desktop-loopback-shell.md)）。
