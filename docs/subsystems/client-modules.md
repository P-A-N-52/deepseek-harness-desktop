...
A package joins the table by declaring `dsh.client` (`platform: 'web'`, optional `inject` edges, optional `immediately`) in its package.json and exporting its built bundle at `exports["./client"]`. Package metadata resolves through `ctx.appModuleResolver`, the package tree selected by app boot for bare plugin imports. In an extensible launch that tree is the profile installation; in a sealed runtime it is the executable closure, while the included tree's `ctx.baseUrl` remains the physical profile directory for relative config rows.
