...
Use `--skip-build` when the required `lib/` artifacts already exist, or `--targets=node24.19.0-linux-x64,node24.19.0-linux-arm64,node24.19.0-macos-arm64` to select platforms. Products land in `dist-exe/` and the script syncs the selected carriers into `python/sdk-runtime/`. macOS builds also sync the matching spawn helper required by `node-pty`.
